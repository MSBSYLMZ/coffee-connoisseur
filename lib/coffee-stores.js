...
export async function getListOfCoffeeStorePhotos() {
	...
	const photos = unsplashResult.response.results.map(result => result.urls["regular"]);
	return photos;
}

const getUrlForCoffeStores = (query, limit, latLong) => {
	return `https://api.foursquare.com/v3/places/search?radius=${DEFAULT_RADIUS}&query=${query}&limit=${limit}${latLong ? "&ll=" + latLong : ""}`;
};
